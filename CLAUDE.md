...
Single-page "Image Combiner" app behind a $9.99/month Stripe subscription: the user uploads two images, the browser POSTs them to a same-origin Vercel function, which forwards them to an n8n webhook and streams back the combined image. Plain static HTML/CSS/vanilla JS plus one Vercel function — no framework, no build step, no dependencies, no test suite. Deployed to Vercel (Framework: "Other", no build command).
...
- `api/generate.mjs` is the image-generation Vercel function (Web-standard `export async function POST(request)`). It rejects cross-origin requests, requires a valid Supabase access token (`Authorization: Bearer`, verified against `SUPABASE_URL/auth/v1/user`, 401 otherwise), requires an `active`/`trialing` row in `public.subscriptions` (queried with the user's own token under RLS, 402 otherwise), re-validates both uploads (size cap + magic-byte type detection), forwards them to `N8N_WEBHOOK_URL` with an optional `X-Webhook-Secret` header, and streams the image back. Error responses are JSON `{ error }`, and `app.js` shows that message to the user.
- Payments: a $9.99/month subscription via a Stripe Payment Link (`PAYMENT_LINK_URL` in `app.js`, opened with `client_reference_id=<user id>`) → Stripe webhook → `api/stripe/webhook.mjs` (Vercel function). It verifies the Stripe signature with `STRIPE_WEBHOOK_SECRET`, re-reads the subscription from the Stripe API with `STRIPE_SECRET_KEY` (so out-of-order events can't regress state), and writes to Supabase over REST with `SUPABASE_SERVICE_ROLE_KEY` (server-only; bypasses RLS). `checkout.session.completed` upserts `public.subscriptions` (only for its `PAYMENT_LINK_ID`); `customer.subscription.updated`/`deleted` re-sync `status`. Access = a row with `status` in (`active`, `trialing`). All secrets are env vars (`.env.local` locally, Vercel Environment Variables in production); nothing Stripe-related lives in Supabase Vault. The old `purchases` table (from the former one-time payment) is unused. Schema is in `supabase/migrations/`. This project does **not** auto-grant table privileges to `anon`/`authenticated`/`service_role`, so new tables need explicit `GRANT`s. Stripe is currently in test mode; README "Going live" lists the swap steps.
- Access flow in `app.js`: after any session is established, `checkAccess()` sets `state.access` to `"paid"` (active subscription) or `"none"` from `subscriptions`. `render()` shows `#paywall` (with "Logged in as <email>") above the generator until paid. Unpaid users still see the upload cards, but `canUpload()` blocks the file picker and drops and shows `#upload-locked` explaining they need to pay. On return from Stripe (`?checkout=success`) it polls until the webhook lands. A 402 from `/api/generate` sets `access = "none"` without clearing images.
...
- `app.js` uses a single `state` object (`image1`, `image1Preview`, `image2`, `image2Preview`, `generatedImage`, `isGenerating`, `error`, plus auth: `session`, `authReady`, `authMode`, `authBusy`, `authError`, `authMessage`, plus access: `access`, `accessChecking`, `paywallError`, `paywallMessage`, `uploadNotice`) and one `render()` function that syncs the whole DOM from state. Mutate state, then call `render()` — don't touch the DOM elsewhere.
...
- MVP scope only — email/password login and a monthly subscription are the only account features; the only table in use is `subscriptions`; no history, prompts, navigation, or extra features.
