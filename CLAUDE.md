...
- `api/generate.mjs` is the only Vercel server code (Web-standard `export async function POST(request)`). It rejects cross-origin requests, requires a valid Supabase access token (`Authorization: Bearer`, verified against `SUPABASE_URL/auth/v1/user`, 401 otherwise), requires a `paid` row in `public.purchases` (queried with the user's own token under RLS) or else claims the user's one free demo via `rpc/claim_free_demo` (402 once it's used), and after n8n responds calls `rpc/finish_free_demo(DEMO_SERVER_SECRET, success)` to mark it used or release it on failure, re-validates both uploads (size cap + magic-byte type detection), forwards them to `N8N_WEBHOOK_URL` with an optional `X-Webhook-Secret` header, and streams the image back. Error responses are JSON `{ error }`, and `app.js` shows that message to the user.
...
- Free demo: `public.free_demos` (one row per user, `pending` → `used`). `finish_free_demo` checks its secret against Vault `demo_server_secret`, which must equal the `DEMO_SERVER_SECRET` env var; without it, users could release their own claim and loop free generations.
- Access flow in `app.js`: after any session is established, `checkAccess()` sets `state.access` to `"paid"`, `"demo"` or `"none"` from `purchases` + `free_demos`. `render()` shows the generator for paid/demo (plus a "first generation is free" note), and after the demo keeps the result visible with an upsell in place of Generate; otherwise `#paywall`. On return from Stripe (`?checkout=success`) it polls until the webhook lands. A 402 from `/api/generate` sets `access = "none"` without clearing images.
...
- MVP scope only — email/password login, one free demo generation and a one-time paid unlock are the only account features; the only tables are `purchases` and `free_demos`; no history, prompts, navigation, or extra features.
