...
- Payments: a Stripe Payment Link (`PAYMENT_LINK_URL` in `app.js`, opened with `client_reference_id=<user id>`) → Stripe webhook → `api/stripe-webhook.mjs` (Vercel function). It verifies the Stripe signature with `STRIPE_WEBHOOK_SECRET` and writes to Supabase over REST with `SUPABASE_SERVICE_ROLE_KEY` (server-only; bypasses RLS). It upserts `public.purchases` on `checkout.session.completed` / `async_payment_succeeded`, and sets `status = 'refunded'` on a full `charge.refunded`. It only accepts checkouts whose `payment_link` equals its `PAYMENT_LINK_ID`. Both secrets are env vars (`.env.local` locally, Vercel Environment Variables in production); nothing Stripe-related lives in Supabase Vault. Schema is in `supabase/migrations/`. This project does **not** auto-grant table privileges to `anon`/`authenticated`/`service_role`, so new tables need explicit `GRANT`s. Stripe is currently in test mode; README "Going live" lists the swap steps.
- Access flow in `app.js`: after any session is established, `checkAccess()` sets `state.access` to `"paid"` or `"none"` from `purchases`. `render()` shows `#paywall` (with "Logged in as <email>") above the generator until paid. Unpaid users still see the upload cards, but `canUpload()` blocks the file picker and drops and shows `#upload-locked` explaining they need to pay. On return from Stripe (`?checkout=success`) it polls until the webhook lands. A 402 from `/api/generate` sets `access = "none"` without clearing images.
